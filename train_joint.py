...
from models.VCNet import MPN, RIN, Discriminator, PatchDiscriminator
from loss import AdaptiveBCELoss, SemanticConsistencyLoss, IDMRFLoss, AdversarialLoss
...
def train(args, cfg):
    ...
    optimizer_disc = optim.Adam(
        params=list(disc.parameters()) + list(pdisc.parameters()),
        lr=cfg.train.optim_disc.lr, betas=cfg.train.optim_disc.betas,
    )
    step = 0
    ...
    def load_pretrained(model_path: str):
        ...
        logger.info(f'Successfully load mpn and rin from {model_path}')
    ...
    def load_ckpt(ckpt_path: str):
        ...
        pdisc.load_state_dict(ckpt_model['pdisc'])
        logger.info(f'Successfully load mpn, rin, disc and pdisc from {ckpt_path}')
        # load optimizers
        ...
        optimizer_disc.load_state_dict(ckpt_optimizer['optimizer_disc'])
    ...
    def save_ckpt(save_path: str):
        ...
        model_state_dicts = dict(
            mpn=accelerator.unwrap_model(mpn).state_dict(),
            rin=accelerator.unwrap_model(rin).state_dict(),
            disc=accelerator.unwrap_model(disc).state_dict(),
            pdisc=accelerator.unwrap_model(pdisc).state_dict(),
        )
    ...
    if cfg.train.pretrained is not None:
        ...
        logger.warning(f'The joint training phase expects to load the pretrained model weights from '
                       f'separate training phase, but cfg.train.pretrained is None. '
    ...
    mpn, rin, disc, pdisc = accelerator.prepare(mpn, rin, disc, pdisc)  # type: ignore
    optimizer_mpn, optimizer_rin, optimizer_disc = accelerator.prepare(optimizer_mpn, optimizer_rin, optimizer_disc)  # type: ignore
    ...
    adv = AdversarialLoss(disc)
    adv_patch = AdversarialLoss(pdisc)
    ...
    def run_step_gen(_batch):
        ...
        loss_meter = metrics.KeyValueAverageMeter(
            keys=['loss_mpn', 'loss_rec', 'loss_semantic', 'loss_idmrf', 'loss_adv_gen'],
        )
        for i in range(0, batch_size, micro_batch):
            ...
            with cm1, cm2:
                ...
                # train rin: use predicted mask in joint training
                ...
                loss_adv_gen = (adv.forward_G(fake_img=rec_img) +
                                adv_patch.forward_G(fake_img=rec_img, mask=mask))
                loss_rin = (cfg.train.coef_rec * loss_rec +
                            cfg.train.coef_semantic * loss_semantic +
                            cfg.train.coef_idmrf * loss_idmrf +
                            cfg.train.coef_adv * loss_adv_gen)
                loss_gen = cfg.train.coef_mpn * loss_mpn + loss_rin
                accelerator.backward(loss_gen * loss_scale)
            loss_meter.update(dict(
                ...
                loss_adv_gen=loss_adv_gen.detach(),
            ), cor_img.shape[0])
        optimizer_mpn.step()
        optimizer_rin.step()
        return dict(
    ...
    def run_step_disc(_batch):
        optimizer_disc.zero_grad()
        batch_cor_img, batch_gt_img, batch_noise, batch_mask = _batch
        batch_size = batch_cor_img.shape[0]
        loss_meter = metrics.KeyValueAverageMeter(keys=['loss_adv_disc'])
        for i in range(0, batch_size, micro_batch):
            cor_img = batch_cor_img[i:i+micro_batch].float()
            gt_img = batch_gt_img[i:i+micro_batch].float()
            mask = batch_mask[i:i+micro_batch].float()
            loss_scale = cor_img.shape[0] / batch_size
            no_sync = (i + micro_batch) < batch_size
            cm1 = accelerator.no_sync(disc) if no_sync else nullcontext()
            cm2 = accelerator.no_sync(pdisc) if no_sync else nullcontext()
            with cm1, cm2:
                pred_mask, bottleneck = mpn(cor_img)
                # train rin: use predicted mask in joint training
                rec_img = rin(cor_img, pred_mask, bottleneck.detach())
                loss_adv_disc = (adv.forward_D(real_img=gt_img, fake_img=rec_img.detach()) +
                                 adv_patch.forward_D(real_img=gt_img, fake_img=rec_img.detach(), mask=mask))
                accelerator.backward(loss_adv_disc * loss_scale)
            loss_meter.update(dict(loss_adv_disc=loss_adv_disc.detach()))
        optimizer_disc.step()
        return dict(**loss_meter.avg, lr_disc=optimizer_disc.param_groups[0]['lr'])
    ...
    def evaluate(dataloader):
        ...
        for cor_img, gt_img, noise, mask in tqdm.tqdm(
            ...
            pred_mask, bottleneck = mpn(cor_img)
            # use predicted mask in joint training
            rec_img = rin(cor_img, pred_mask, bottleneck.detach())
    ...
    def sample(savepath: str):
        ...
        # use predicted mask in joint training
        rec_img = unwrapped_rin(cor_img, pred_mask, bottleneck.detach())
    ...
    while step < cfg.train.n_steps:
        ...
        # run a step
        mpn.train(); rin.train()
        disc.train(); pdisc.train()
        train_status = run_step_disc(batch)
        status_tracker.track_status('Train', train_status, step)
        train_status = run_step_gen(batch)
        status_tracker.track_status('Train', train_status, step)
        accelerator.wait_for_everyone()
